...
/*
    ...
    1) Controller(Presentation 계층)
        : HTTP 요청을 받음
        - 입력 DTO를 검증(@Valid)하고, 비지니스 로직은 전부 Service에 위임
        - 일관된 응답 포멧으로 감싸서 반환 (ResponseEntity<ResponseDto<?>>)
    ...
    2) Service(Business 계층)
        ...
        +) DTO << - >> 엔티티 매핑도 발생
        +) createdAt/updatedAt 은 엔티티의 @PrePersist/@PreUpdate 대신 서비스 레벨에서 관리

    3) Repository(Persistence 계층)
        : JPA를 통해 순수한 DB 접근만 담당
        - 쿼리 메서드, QueryDSL 등을 정의하고 비지니스 로직은 절대 작성 x
