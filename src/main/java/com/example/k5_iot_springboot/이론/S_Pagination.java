...
*   - totalElements/totalPages: 전체 개수/전체 페이지수 (Offset 방식에서만 사용)
*   - Page VS Slice
*   >> Page: 전체 개수(count) 포함 ("총 몇페이지인지" 같은 정보가 필요할 때)
*   >> Size: 전체 개수 없이 "다음 페이지가 더 있는지(hasNext)"만 판단
...
*       >> 데이터가 클 경우 (수십만 ~ 수천만 건), 무한 스크롤/피드형 목록, 실시간성의 성능의 중요한 화면)
*
*       +) 마지막으로 번 키 이후 데이터를 키 조건(id < lastId)로 조회
*           앞부분을 통째로 건너뛰지 않음.
...
*       - Query: cursorId(저음 호출 시 생략 - 최신부터), size(기본 10, 최대 100 권장)
*       - 응답: content(목록) + hashNext + nextCursor (다음 요청에 바로 사용)
*
* */
