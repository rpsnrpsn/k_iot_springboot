...
/*
    ...
    1. 서블릿이란
        ...
        - 웹 서버(WAS)에 배치
        - HTTP 프로토콜을 통해 웹 페이지 생성 | 데이터 처리 후 응답 반환

        +) JAVA EE(Enterprise Edition) 플램폼의 일부
            >> 대부분의 자바 웹 프레임워크들이 servlet 기반으로 동작

    2) 서블릿 동작 구조
        : 요청 >> 컨테이너 >> 서블릿 >> 비지니스 로직 >> 응다
        ...
        [ 서블릿 ] service() 호출 - doGet()/doPost() - 필요한 작업 수행
            |
        [ 서비스 ] DAO | 리포지토리 | DB ] 비지니스 로직, 데이터 접근
            |
            | : 결과(Model)
            |
        ...
        3) 순수 서블릿은 요청 피싱, 모델 구성, 뷰 렌더링을 개발자가 직접 처리 - 중복 코드 증가
