...
public class D_Post {
    ...
    @OneToMany(
            ...
            orphanRemoval = true,
            // : 컬렉션에서 제거된 댓글은 고아 객체로 간주되어, DB에서 자동 삭제 (실제 DELETE 수행)
    ...
    public void chanceContent(String content) { this.content = content; }
    ...
    * - comments 리스트에 추가/삭제
    *   >> 해당 Comment에 post 필드에 현재 Post 객체를 설정
    ...
    *   - Post가 여러개의 Comment를 가짐 (@OneToMany)
