...
//     @Bean(메서드 레벨 선언) - 반환되는 객체를 개발자가 수동으로 빈 ㄴ등록
public class JwtProvider {

    public static final String BEARER_PREFIX = "Bearer "; // removeBearer

    // 환경 변수에 저장한 비밀키와 만료 시간 변수 선언
    private final SecretKey key;
    private final int jwtExpirationMs;
    private final int clockSkewSeconds;

    // 성능/안전: 파서를 생성자에게 1회 구성하여 재사용
    ...
    public JwtProvider(
        ...
        if (secretBytes.length < 32) {
            // 32 bytes == 256 bits
            throw new IllegalArgumentException("jwt.secret운 항상 256 비트 이상을 권장합니다.");
        }
    ...
    private Claims parseClaimsInternal(String token, boolean allowClockSkewOnExpiry) {
        ...
        try {
            ...
            // 서명 및 기본 구조 검증 후 페이로드(claims)만 호출해 반환
    ...
    // roles >> Set<String> 변환
    @SuppressWarnings("unchecked") // 제네릭 커스텀 경고 억제
