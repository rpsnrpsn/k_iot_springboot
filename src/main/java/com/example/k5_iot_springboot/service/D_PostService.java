...
public interface D_PostService {
    ...
    ResponseDto<List<PostListResponseDto>> searchPostByCommentKeyword(@NotBlank(message = "검색 키워드는 비어있을 수 없습니다.") String keyword);

    ResponseDto<List<PostWithCommentCountResponseDto>> getAuthorPostWithMinComments(@NotBlank(message = "작성자(author)는 비워질 수 없습니다.") String author, @PositiveOrZero(message = "minCount는 0 이상이어야 합니다.") int minCount);
}