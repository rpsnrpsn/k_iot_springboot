...
CREATE TABLE IF NOT EXISTS `orders` (
    ...
    CONSTRAINT fk_orders_user
		FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
	CONSTRAINT chk_orders_os CHECK (order_status IN ('PENDINT', 'APPROVED', 'CANCELLED')),
...
CREATE TABLE IF NOT EXISTS `order_items` (
    ...
    created_at	DATETIME(6) NOT NULL,
    updated_at	DATETIME(6) NOT NULL,
    CONSTRAINT fk_order_tiems_order
...
CREATE TABLE IF NOT EXISTS `order_logs` (
    ...
    message		VARCHAR(255),
    -- 트리거가 직접 INSERT 하는 로고 테이블은 시간 누락 방지를 위해 DB 기본값 유지
...
BEGIN
    ...
    VALUES (NEW.id, CONCATT('주문이 생성되었습니다. 주문 ID: ', NEW.id));
END //
DELIMITER ;
...
FOR EACH ROW
BEGIN
	IF NEW.order_status <> OLD.order_status THEN -- A <> B 는 A != B 와 같은 의미 (같지 않다)
		INSERT INTO order_logs(order_id, message)
